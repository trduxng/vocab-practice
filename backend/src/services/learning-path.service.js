...
class LearningPathService {
  ...
  static async createSchema() {
    ...
    for (const level of LEVEL_SEED) {
      ...
      await pool.request()
        .input("LevelCode", sql.NVarChar(30), code)
        .input("LevelName", sql.NVarChar(100), name)
        .input("TargetScore", sql.Int, targetScore)
        .input("Description", sql.NVarChar(500), description)
        .input("DisplayOrder", sql.Int, displayOrder)
        .input("AccentKey", sql.NVarChar(30), accentKey)
        .query(`
          MERGE dbo.LearningPathLevels AS target
          USING (SELECT @LevelCode AS LevelCode) AS source
          ON target.LevelCode = source.LevelCode
          WHEN MATCHED THEN
            UPDATE SET LevelName = @LevelName,
      ...
      .query(`
        ...
        OUTER APPLY (
          ...
          LEFT JOIN dbo.MiniTestAttempts mta
           ...
           AND mta.UserID = @UserID
          WHERE mt.TopicID = t.TopicID
            AND mt.IsPublished = 1
        ) testStats
        WHERE t.ContentStatus = N'Published'
  ...
  static buildRoadmap(levelRows, topicRows) {
    ...
    const levels = levelRows.map((level) => {
      const topics = topicRows
        .filter((topic) => Number(topic.levelId) === Number(level.id))
        .map((topic) => {
          ...
          const completionPercentage = Math.round(
            completionParts.reduce((sum, progress) => sum + progress, 0) / completionParts.length,
          );
            ...
            },
            {
              ...
              status: miniTestCompleted
                ...
                : canPractice && practiceCompleted && miniTestConfigured
                  ? "available"
                  : "locked",
              route: topic.firstMiniTestId ? `/user/minitests/${topic.firstMiniTestId}` : "/user/minitests",
              configured: miniTestConfigured,
            },
          ];

          const pathTopic = {
            pathTopicId: Number(topic.pathTopicId),
      ...
      const availableTopics = topics.filter((topic) => topic.status === "available").length;
      return {
        id: Number(level.id),
        code: level.code,
        title: level.title,
        targetScore: Number(level.targetScore),
        description: level.description,
        displayOrder: Number(level.displayOrder),
        accentKey: level.accentKey,
        status: topics.length > 0 && completedTopics === topics.length
          ? "completed"
          : availableTopics > 0
            ? "available"
            : "locked",
        completionPercentage: topics.length > 0
          ? Math.round(topics.reduce((sum, topic) => sum + topic.completionPercentage, 0) / topics.length)
          : 0,
        completedTopics,
    ...
    const currentTopicIndex = allTopics.findIndex((topic) => topic.status === "available");
    ...
    const currentActivity = currentTopic?.activities.find((activity) => activity.status === "available") || null;

    return {
      completionPercentage: allTopics.length > 0
        ? Math.round(allTopics.reduce((sum, topic) => sum + topic.completionPercentage, 0) / allTopics.length)
        : 0,
      completedTopics,
      totalTopics: allTopics.length,
      currentPosition: currentTopic ? {
        levelTitle: levels.find((level) => level.topics.some((topic) => topic.topicId === currentTopic.topicId))?.title,
      ...
      currentLesson: currentTopic ? {
        ...
        route: `/user/learn/${currentTopic.topicId}`,
      ...
      nextLesson: nextTopic ? {
        ...
        route: `/user/learn/${nextTopic.topicId}`,
        status: nextTopic.status,
        completionPercentage: nextTopic.completionPercentage,
      } : null,
      levels,
    };
  }
