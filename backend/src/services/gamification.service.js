...
class GamificationService {
  ...
  static async awardXP(userId, { eventType, amount, sourceKey = null, metadata = null, xpAmount = null } = {}) {
    await this.ensureSchema();
    const baseAmount = XP_REWARDS[eventType];
    const finalAmount = xpAmount ?? amount ?? baseAmount;
    if (finalAmount == null) throw new Error(`Unsupported XP event type: ${eventType}`);
