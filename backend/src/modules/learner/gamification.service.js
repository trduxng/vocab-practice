...
class GamificationService {
  ...
  static async createSchema() {
    ...
    await pool.request().query(`
      IF COL_LENGTH(N'dbo.Users', N'TotalXP') IS NULL
        ALTER TABLE dbo.Users ADD TotalXP INT NOT NULL CONSTRAINT DF_Users_TotalXP DEFAULT (0);
      IF COL_LENGTH(N'dbo.Users', N'CurrentLevel') IS NULL
        ALTER TABLE dbo.Users ADD CurrentLevel INT NOT NULL CONSTRAINT DF_Users_CurrentLevel DEFAULT (1);
      ...
      END;
      IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'UX_UserXPEvents_User_Event_Source' AND object_id = OBJECT_ID(N'dbo.UserXPEvents'))
        CREATE UNIQUE INDEX UX_UserXPEvents_User_Event_Source ON dbo.UserXPEvents(UserID, EventType, SourceKey) WHERE SourceKey IS NOT NULL;
      IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_UserXPEvents_UserID_CreatedAt' AND object_id = OBJECT_ID(N'dbo.UserXPEvents'))
        CREATE INDEX IX_UserXPEvents_UserID_CreatedAt ON dbo.UserXPEvents(UserID, CreatedAt DESC) INCLUDE (EventType, XPAmount);
      ...
      IF COL_LENGTH(N'dbo.Achievements', N'XPReward') IS NULL
        ALTER TABLE dbo.Achievements ADD XPReward INT NOT NULL CONSTRAINT DF_Achievements_XPReward DEFAULT (50);
    ...
    for (const achievement of ACHIEVEMENT_SEED) {
      ...
      await pool.request()
        .input("Code", sql.NVarChar(80), code).input("Name", sql.NVarChar(200), name)
        .input("Description", sql.NVarChar(500), description).input("Icon", sql.NVarChar(20), icon)
        .input("CriteriaType", sql.NVarChar(50), criteriaType).input("CriteriaValue", sql.Int, criteriaValue)
        .input("DisplayOrder", sql.Int, displayOrder).input("XPReward", sql.Int, xpReward || 50)
        .query(`
          MERGE dbo.Achievements AS target USING (SELECT @Code AS Code) AS source ON target.Code = source.Code
          WHEN MATCHED THEN UPDATE SET Name = @Name, Description = @Description, Icon = @Icon,
            CriteriaType = @CriteriaType, CriteriaValue = @CriteriaValue, DisplayOrder = @DisplayOrder,
            XPReward = @XPReward, IsActive = 1
          WHEN NOT MATCHED THEN INSERT (Code, Name, Description, Icon, CriteriaType, CriteriaValue, DisplayOrder, XPReward)
  ...
  static async awardXP(userId, { eventType, sourceKey = null, metadata = null, xpAmount = null } = {}) {
    ...
    try {
      ...
      const result = await request
        .input("UserID", sql.BigInt, userId).input("EventType", sql.NVarChar(50), eventType)
        .input("XPAmount", sql.Int, amount).input("SourceKey", sql.NVarChar(200), sourceKey)
        ...
        .query(`
          ...
          IF @SourceKey IS NULL OR NOT EXISTS (SELECT 1 FROM dbo.UserXPEvents WHERE UserID = @UserID AND EventType = @EventType AND SourceKey = @SourceKey)
          BEGIN
            ...
            UPDATE dbo.Users SET TotalXP = ISNULL(TotalXP, 0) + @XPAmount, UpdatedAt = SYSDATETIMEOFFSET() WHERE UserID = @UserID;
          END;
      ...
      await new sql.Request(transaction).input("UserID", sql.BigInt, userId)
        .input("CurrentLevel", sql.Int, level.currentLevel)
        .query(`UPDATE dbo.Users SET CurrentLevel = @CurrentLevel WHERE UserID = @UserID`);

      await transaction.commit();
      const unlockedAchievements = (row.xpGained > 0 && eventType !== "AchievementUnlock") ? await this.checkAchievements(userId) : [];
      return {
        xpEventId: Number(row.xpEventId || 0), xpGained: Number(row.xpGained || 0),
        eventType, awarded: Number(row.xpGained || 0) > 0, ...level, unlockedAchievements,
      };
    } catch (error) {
  ...
  static async checkAchievements(userId) {
    ...
    const result = await pool.request()
      .input("UserID", sql.BigInt, userId).input("WordsLearned", sql.Int, profile.wordsLearned)
      .input("StreakDays", sql.Int, profile.streak).input("BestTestScore", sql.Int, profile.bestTestScore)
      ...
      .query(`
        ...
        SELECT @UserID, a.AchievementID FROM dbo.Achievements a WHERE a.IsActive = 1
        AND NOT EXISTS (SELECT 1 FROM dbo.UserAchievements ua WHERE ua.UserID = @UserID AND ua.AchievementID = a.AchievementID)
        AND ((a.CriteriaType = N'WORDS_LEARNED' AND @WordsLearned >= a.CriteriaValue)
    ...
    const newlyUnlocked = allAchievements.filter((a) => unlockedIds.includes(a.id));
    ...
    for (const ach of newlyUnlocked) {
      ...
      await this.awardXP(userId, {
        ...
        xpAmount: rewardXP,
      }).catch((err) => console.error(`[GamificationService] Error awarding XP for ${ach.code}:`, err));
    }
  ...
  static async getMetrics(userId) {
    ...
    const result = await pool.request().input("UserID", sql.BigInt, userId).query(`
      WITH DailyActivity AS (
        SELECT DISTINCT CAST(CreatedAt AS DATE) AS ActivityDate FROM dbo.UserXPEvents
        WHERE UserID = @UserID AND EventType IN (N'LearnWord', N'PracticeComplete', N'MiniTestComplete', N'DailyLogin')
        UNION SELECT DISTINCT CAST(AttemptedAt AS DATE) AS ActivityDate FROM dbo.ExerciseAttempts WHERE UserID = @UserID
      ),
      RankedActivity AS (
        SELECT ActivityDate, MAX(ActivityDate) OVER () AS LatestDate,
               ROW_NUMBER() OVER (ORDER BY ActivityDate DESC) AS rowNumber FROM DailyActivity
      )
      SELECT ISNULL((SELECT TotalXP FROM dbo.Users WHERE UserID = @UserID), 0) AS totalXP,
             ...
             ISNULL((SELECT COUNT(*) FROM RankedActivity WHERE LatestDate >= DATEADD(day, -1, CAST(SYSDATETIMEOFFSET() AS DATE)) AND DATEDIFF(day, ActivityDate, LatestDate) = rowNumber - 1), 0) AS streak,
             ISNULL((SELECT SUM(XPAmount) FROM dbo.UserXPEvents WHERE UserID = @UserID AND CAST(CreatedAt AS DATE) = CAST(SYSDATETIMEOFFSET() AS DATE)), 0) AS todayXP;
    `);
    const row = result.recordset[0] || {};
    return {
      wordsLearned: Number(row.wordsLearned || 0), bestTestScore: Number(row.bestTestScore || 0),
      streak: Number(row.streak || 0), todayXP: Number(row.todayXP || 0),
  ...
  static async getAchievements(userId, metrics = null) {
    ...
    const pool = await poolPromise;
    const result = await pool.request().input("UserID", sql.BigInt, userId).query(`
      SELECT a.AchievementID AS id, a.Code AS code, a.Name AS label, a.Description AS description,
             a.Icon AS icon, a.CriteriaType AS criteriaType, a.CriteriaValue AS target,
             a.XPReward AS xpReward, CASE WHEN ua.UserAchievementID IS NULL THEN CAST(0 AS BIT) ELSE CAST(1 AS BIT) END AS unlocked,
             ua.UnlockedAt AS unlockedAt, CASE WHEN ua.SeenAt IS NULL THEN CAST(0 AS BIT) ELSE CAST(1 AS BIT) END AS seen
      FROM dbo.Achievements a LEFT JOIN dbo.UserAchievements ua ON ua.AchievementID = a.AchievementID AND ua.UserID = @UserID
      WHERE a.IsActive = 1 ORDER BY a.DisplayOrder;
    `);
    return result.recordset.map((achievement) => {
      ...
      return { ...achievement, id: Number(achievement.id), target, progress, progressPercentage: Math.min(100, Math.round((progress / target) * 100)) };
    });
  }
  ...
  static async getProfile(userId) {
    ...
    return { ...metrics, achievements, unseenAchievements: achievements.filter((a) => a.unlocked && !a.seen) };
  }
  ...
  static async markAchievementsSeen(userId, achievementIds = []) {
    ...
    if (!Array.isArray(achievementIds) || achievementIds.length === 0) {
      await request.query(`UPDATE dbo.UserAchievements SET SeenAt = COALESCE(SeenAt, SYSDATETIMEOFFSET()) WHERE UserID = @UserID;`);
      return;
    }
    const safeIds = achievementIds.map(Number).filter(Number.isInteger);
    if (safeIds.length === 0) return;
    await request.query(`UPDATE dbo.UserAchievements SET SeenAt = COALESCE(SeenAt, SYSDATETIMEOFFSET()) WHERE UserID = @UserID AND AchievementID IN (${safeIds.join(",")});`);
  }
}
