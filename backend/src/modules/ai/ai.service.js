...
function normalizeExamples(examples) {
  ...
  return examples.map((e) => ({ sentence: cleanText(e?.sentence), meaning: cleanText(e?.meaning) })).filter((e) => e.sentence).slice(0, 5);
}
...
class AiService {
  static async suggestWordContent({ term, meaning, partOfSpeech, exampleCount = 3 }) {
    ...
    return {
      ...
      sources: { ai: 'openai', translate: translatedTerm ? 'google-translate' : null, dictionary: dictionary ? 'dictionaryapi.dev' : null },
    };
  }

  static async generateToeicExamples({ term, meaning, partOfSpeech, exampleCount }) {
    const apiKey = process.env.OPENAI_API_KEY;
    if (!apiKey) { const err = new Error('OPENAI_API_KEY is not configured'); err.statusCode = 503; throw err; }
    ...
    try {
      const response = await fetch(OPENAI_RESPONSES_URL, {
        ...
        body: JSON.stringify({
          ...
          input: JSON.stringify({ term, knownVietnameseMeaning: meaning, knownPartOfSpeech: partOfSpeech, exampleCount, outputShape: { meaning: 'Vietnamese meaning, concise', phonetic: 'IPA if confident, otherwise empty string', partOfSpeech: 'English part of speech if confident', examples: [{ sentence: 'English TOEIC sentence', meaning: 'Vietnamese translation' }] } }),
      ...
      if (!response.ok) {
        ...
        const err = new Error(message); err.upstreamStatusCode = response.status;
        err.statusCode = response.status === 401 || response.status === 403 ? 502 : response.status; throw err;
      }
  ...
  static async translateToVietnamese(text) {
    ...
    try {
      const response = await fetch(`${GOOGLE_TRANSLATE_URL}?key=${encodeURIComponent(apiKey)}`, { method: 'POST', signal, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ q: text, source: 'en', target: 'vi', format: 'text' }) });
      const data = await response.json().catch(() => ({}));
  ...
  static async lookupDictionary(term) {
    ...
    try {
      ...
      const phonetic = cleanText(firstEntry.phonetic) || cleanText((firstEntry.phonetics || []).find((item) => cleanText(item.text))?.text);
      const firstMeaning = (firstEntry.meanings || [])[0];
      const firstDefinition = (firstMeaning?.definitions || [])[0];
      const examples = [];
      for (const meaning of firstEntry.meanings || []) {
        for (const definition of meaning.definitions || []) {
          if (definition.example) examples.push({ sentence: definition.example, meaning: '' });
        }
      }
      return { phonetic, partOfSpeech: cleanText(firstMeaning?.partOfSpeech), meaning: cleanText(firstDefinition?.definition), examples: normalizeExamples(examples) };
    } finally { clear(); }
  }
}
