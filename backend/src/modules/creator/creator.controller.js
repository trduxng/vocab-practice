...
class CreatorController {
  ...
  static async submitMiniTestForReview(req, res, next) { try { const ok = await CreatorService.submitForReview('minitest', req.params.id, req.user.id); if (!ok) return res.status(400).json({ message: 'Không thể gửi duyệt' }); res.json({ message: 'Đã gửi duyệt' }); } catch (e) { next(e); } }
