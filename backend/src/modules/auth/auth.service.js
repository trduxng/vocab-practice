...
const GamificationService = require('../learner/gamification.service');
...
class AuthService {
  static async register(fullName, email, password, role) {
    ...
    const permResult = await pool.request()
      ...
      .query(`
        SELECT p.PermissionCode FROM RolePermissions rp
  ...
  static async login(email, password) {
    ...
    const permissions = permResult.recordset.map(r => r.PermissionCode);
    const payload = { id: user.id, fullName: user.fullName, role: user.role, permissions };
    ...
    return {
      token,
      gamification,
      user: { id: user.id, fullName: user.fullName, email: user.email, role: user.role, permissions }
    };
  }
}
