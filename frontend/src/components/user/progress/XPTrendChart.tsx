...
function buildSeries(activity: ProgressActivityDay[], period: TrendPeriod) {
  if (period === "daily") {
    return activity.slice(-30).map((day) => ({ label: shortDate(day.date), xp: day.xpEarned }));
  }
  ...
  return Array.from(monthMap.entries()).slice(-12).map(([month, xp]) => ({
      ...
      {totalXP === 0 ? (
        ...
        <div className="mt-6 h-64 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={series} margin={{ top: 8, right: 4, left: -22, bottom: 0 }}>
              <defs>
