  ...
  return (
    <section className="rounded-[28px] border border-slate-200 bg-white px-3 py-3 shadow-sm dark:border-white/10 dark:bg-white/[0.04] sm:px-4 sm:py-3">
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-amber-100 text-amber-600 dark:bg-amber-500/15 dark:text-amber-300">
      ...
      <div className="mt-3">
        {unlocked.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/70 px-3 py-4 text-center dark:border-white/10 dark:bg-white/[0.02]">
            <Award className="mx-auto h-6 w-6 text-slate-300 dark:text-slate-600" />
            <p className="mt-2 text-xs font-bold text-slate-700 dark:text-slate-200">Huy hiệu đầu tiên đang chờ bạn</p>
            <p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">Bắt đầu một bài học để mở khóa thành tích.</p>
          </div>
        ) : (
          <div className="grid grid-cols-3 gap-1.5">
            {unlocked.map((achievement) => (
              <div key={achievement.id} className="rounded-2xl border border-amber-100 bg-amber-50/70 p-2 text-center dark:border-amber-500/15 dark:bg-amber-500/[0.06]">
                <div className="text-xl" aria-hidden="true">{achievement.icon}</div>
                <p className="mt-1 truncate text-[10px] font-black text-slate-800 dark:text-slate-100" title={achievement.label}>
                  {achievement.label}
                </p>
                <p className="mt-0.5 text-[8px] font-bold uppercase tracking-wider text-amber-600 dark:text-amber-400">Đã mở khóa</p>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="mt-3 rounded-2xl border border-slate-100 bg-slate-50/70 p-3 dark:border-white/5 dark:bg-white/[0.025]">
        {nextAchievement && nextProgress ? (
          <>
            <div className="flex items-center gap-2">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-200 text-slate-500 dark:bg-white/10 dark:text-slate-400">
                <LockKeyhole className="h-3.5 w-3.5" />
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-[9px] font-bold uppercase tracking-[0.16em] text-slate-400">Thành tích tiếp theo</p>
                <p className="truncate text-xs font-black text-slate-800 dark:text-slate-100">{nextAchievement.label}</p>
              </div>
              <span className="text-[10px] font-black text-slate-600 dark:text-slate-300">{percentage}%</span>
            </div>
            ...
            <p className="mt-1 text-[10px] text-slate-500 dark:text-slate-400">{progressDetail}</p>
          </>
        ) : (
          <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400">Bạn đã mở khóa toàn bộ huy hiệu hiện có.</p>
        )}
      </div>
      ...
      <button
        type="button"
        onClick={onViewAll}
        className="mt-4 inline-flex items-center gap-2 text-xs font-black uppercase tracking-wider text-slate-600 transition-colors hover:text-amber-600 dark:text-slate-300 dark:hover:text-amber-300"
      >
