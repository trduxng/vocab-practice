...
const StudentFlashcard = () => {
  ...
  if (authLoading || loading) {
    return <div className="min-h-screen flex items-center justify-center bg-[#0a0f1e] text-white font-mono">LOADING LESSON...</div>;
  }
  ...
  if (flashcards.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#0a0f1e] text-white p-6 text-center">
        ...
        <Button onClick={() => router.push('/user/dashboard')} className="bg-blue-600 rounded-xl px-8 h-12 font-bold uppercase text-[10px] tracking-widest">Quay về Dashboard</Button>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#0a0f1e] text-white p-6 text-center animate-in fade-in duration-500">
        ...
        <div className="flex gap-4">
          <Button variant="outline" onClick={() => window.location.reload()} className="border-white/10 text-white h-12 px-8 rounded-xl font-bold uppercase text-[10px] tracking-widest">Học tiếp</Button>
          <Button onClick={() => router.push('/user/dashboard')} className="bg-blue-600 h-12 px-8 rounded-xl font-bold uppercase text-[10px] tracking-widest">Xong</Button>
        </div>
      </div>
    );
  }
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="mb-12">
          ...
          <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden border border-white/5 p-[1px]">
            <div
              className="h-full bg-linear-to-r from-blue-600 to-cyan-400 transition-all duration-500 shadow-glow"
        ...
        {card && (
          <div className="relative" style={{ perspective: "2000px" }}>
            ...
            >
              {/* FRONT */}
              <div
                className="absolute inset-0 rounded-[48px] bg-linear-to-br from-[#1a2333] to-[#0d1526] border border-white/10 flex flex-col items-center justify-center shadow-2xl"
              ...
              >
                ...
                <h2 className="text-white text-7xl font-black mb-4 tracking-tighter text-center px-6">
                  {card.term}
                </h2>
                ...
                >
                  ...
                  <span className="text-slate-300 text-xs font-black uppercase tracking-widest">Pronunciation</span>
                </div>
              </div>
              ...
              >
                <h2 className="text-white text-5xl font-black mb-8 leading-tight tracking-tighter">
                  {card.meaning}
                </h2>
                <div className="w-16 h-1.5 bg-white/20 rounded-full mb-10" />
                <p className="text-blue-100 text-xl leading-relaxed max-w-md font-medium italic mb-10">
                  "{card.questionText}"
                </p>
                <p className="text-blue-300/40 text-[10px] font-black uppercase tracking-widest">Tap to see word</p>
              </div>
            </div>
          </div>
        )}
