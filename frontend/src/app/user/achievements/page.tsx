...
const UserAchievements = () => {
  ...
  if (authLoading || loading) {
    return <div className="min-h-screen flex items-center justify-center bg-[#0a0f1e] text-white">Đang tải bảng thành tích...</div>;
  }
    ...
    <div className="flex-1 flex flex-col bg-[#0a0f1e]">
      ...
      <main className="p-6 space-y-8 overflow-auto max-w-5xl mx-auto w-full py-12">
        <div className="text-center mb-12">
          ...
          <h1 className="text-white text-3xl font-black uppercase tracking-tighter">Bảng thành tích</h1>
          <p className="text-slate-500 text-sm mt-2">Theo dõi các cột mốc học tập và mở khóa huy hiệu mới.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {stats?.achievements?.map((achievement) => (
            <Card key={achievement.id} className={`bg-white/3 border transition-all rounded-[32px] overflow-hidden ${achievement.unlocked ? "border-amber-500/20 shadow-lg shadow-amber-900/5" : "border-white/5 opacity-50"}`}>
              <CardContent className="p-8 flex items-center gap-6">
                ...
                <div className="flex-1">
                  <h3 className={`font-black uppercase tracking-widest text-sm mb-1 ${achievement.unlocked ? "text-white" : "text-slate-600"}`}>{achievement.label}</h3>
                  <p className="text-slate-500 text-xs leading-relaxed">{achievementDescription(achievement.id)}</p>
        ...
        <div className="bg-white/3 border border-white/8 rounded-[40px] p-10 mt-12 flex flex-col items-center text-center">
          ...
          <h3 className="text-white font-bold mb-2">Thử thách bản thân</h3>
          <p className="text-slate-500 text-sm max-w-sm">Tiếp tục học mỗi ngày để nhận thêm XP và nâng cấp huy hiệu của bạn.</p>
        </div>
      </main>
    </div>
  );
