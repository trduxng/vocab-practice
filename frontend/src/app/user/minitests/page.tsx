...
import { FileText, Play, Clock, BookOpen, ChevronRight } from "lucide-react";
...
const MiniTestsPage = () => {
  ...
  if (authLoading || loading) {
    return <div className="min-h-screen flex items-center justify-center bg-[#080d1a] text-white">Đang tải danh sách bài thi...</div>;
  }

  return (
    <div className="flex-1 flex flex-col bg-[#080d1a]">
      <Topbar title="Bài thi Mini Test" role="student" userName={user?.fullName} />
      ...
      <main className="p-6 space-y-6 overflow-auto max-w-5xl mx-auto w-full">
        <div className="bg-blue-600/10 border border-blue-500/20 rounded-3xl p-8 mb-8 relative overflow-hidden">
           <div className="relative z-10">
              ...
              <p className="text-blue-300/60 text-sm max-w-md">Các bài thi Mini Test giúp bạn tổng hợp kiến thức và đo lường sự tiến bộ thực tế sau mỗi chủ đề.</p>
           </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
           {tests.length > 0 ? tests.map((test) => (
                ...
                <CardContent className="p-0">
                   <div className="p-6">
                      <div className="flex justify-between items-start mb-4">
                         ...
                         <span className="text-[10px] px-2 py-1 rounded bg-white/5 text-slate-500 font-bold uppercase tracking-widest border border-white/5">{test.topicName || "Tổng hợp"}</span>
                      </div>
                      <h3 className="text-white font-bold text-lg mb-2 group-hover:text-blue-400 transition-colors">{test.title}</h3>
                      <p className="text-slate-500 text-xs line-clamp-2 mb-6 h-8">{test.description || "Bài thi đánh giá năng lực từ vựng tổng hợp."}</p>
                      ...
                      <div className="flex items-center gap-4 text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-6">
                         ...
                         <span className="flex items-center gap-1.5"><BookOpen size={12} className="text-blue-500" /> {test.totalQuestions} câu</span>
                      </div>

                      <Button 
                        onClick={() => alert("Tính năng làm bài thi đang được phát triển...")}
                      ...
                      >
                        Bắt đầu thi <ChevronRight size={14} />
                      </Button>
                   </div>
                </CardContent>
             </Card>
             ...
             <div className="col-span-2 py-20 text-center text-slate-600 border border-dashed border-white/10 rounded-3xl">
                ...
                <p>Hiện chưa có bài thi nào được xuất bản.</p>
             </div>
           )}
        </div>
      </main>
