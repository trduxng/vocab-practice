...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterFormValues) => {
    try {
      ...
      toast.error(error.response?.data?.message || 'Email đã tồn tại hoặc dữ liệu không hợp lệ');
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#080d1a] p-4 relative overflow-hidden">
      ...
      <Card className="w-full max-w-md bg-white/5 border-white/10 shadow-2xl rounded-[40px] overflow-hidden backdrop-blur-xl animate-in slide-in-from-bottom-4 duration-500">
        <CardHeader className="pt-12 pb-8 text-center bg-linear-to-b from-white/[0.03] to-transparent">
          ...
          <CardTitle className="text-3xl font-black text-white tracking-tighter uppercase">Tạo tài khoản</CardTitle>
          <p className="text-slate-500 text-sm mt-2">Bắt đầu hành trình chinh phục TOEIC ngay</p>
        </CardHeader>
        ...
        <CardContent className="px-10">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div className="space-y-2">
              ...
              {errors.fullName && <p className="text-red-400 text-[10px] font-bold uppercase ml-1">{errors.fullName?.message}</p>}
            </div>
